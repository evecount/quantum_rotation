...
> * **What It IS (Natural Molecular Kinetics):** In physical chemistry, bond vibrations and electronic rearrangements happen on the **femtosecond ($10^{-15}\text{ s}$)** scale: retinal in rhodopsin, our PDB 1U19 benchmark, isomerizes in about 200 femtoseconds. Q-Rotate does **not** simulate those dynamics. The $\tau$ in $\hat{U}_{\text{tube}}(\tau) = \exp(-i \tau \hat{H}_{\text{tube}})$ is a fixed, dimensionless circuit parameter (0.25 in every benchmark), not a physical time. What the method does is compare molecular shapes: the blind search locks onto the right pose in **1 to 5 quantum iterations** instead of stepping through a grid of orientations.
> * **What It IS NOT (Hardware Wall-Clock Time):** It is **not** a claim that the physical macroscopic quantum computer executes the cloud job in $10^{-15}$ seconds of lab wall-clock time. Physical trapped-ion QPU gates (Quantinuum H2 and Helios) take **microseconds**, and moving ions between gates stretches a circuit layer to **milliseconds**. A 100-shot circuit should take roughly **seconds** of machine time, plus queue time; we have not yet timed one on hardware.
