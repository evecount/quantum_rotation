...
@app.cell
def _():
    mo.md("## 2. Interactive Biomolecular Ingestion (HPC Embedding Bridge)")
    return
...
def _():
    ...
    return misalignment_slider, noise_slider, n_sites_slider
...
def _(misalignment_slider, n_sites_slider, noise_slider):
    ...
    **Phase Discrepancy ($\Delta \Phi$)**: `{[round(d, 3) for d in delta_phi]}`
    ...
    return delta_phi, ligand_geo, ligand_phases, pocket_geo, pocket_phases


@app.cell(hide_code=True)
def _():
    mo.md("## 3. Blind Parity Resonance Curve")
    return
...
@app.cell(hide_code=True)
def _():
    mo.md("## 4. Hardware Compilation & Quantinuum H-Series Native Gates")
    return
...
def _(ligand_phases, pocket_phases):
    ...
    return cost_card, op_counts, raw_circuit, rebased_circuit
...
@app.cell(hide_code=True)
def _():
    mo.md("## 5. Guppy Dynamic Repeat-Until-Success (RUS) Compilation")
    return
...
def _():
    ...
    return hugr, hugr_stats, qir_bytes


@app.cell(hide_code=True)
def _():
    mo.md("## 6. Execution on Quantinuum H2-2E via Aqora")
    return
