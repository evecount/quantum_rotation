...
def _write_js(payload: dict, js_path: str) -> None:
    """Ships the diagnostics to the Constellation as a plain script, so the
    page can show what a larger register actually buys. Same reason as the
    other generated assets: fetch() of a local JSON is blocked on file://."""
    os.makedirs(os.path.dirname(os.path.abspath(js_path)), exist_ok=True)
    with open(js_path, "w", encoding="utf-8") as handle:
        handle.write("// Generated by src/qrotate/encoding_diagnostics.py\n")
        handle.write("// What the phase register can and cannot tell apart.\n")
        handle.write("// Do not edit by hand: re-run `python -m src.qrotate.encoding_diagnostics`.\n")
        handle.write("window.QROTATE_ENCODING_DIAGNOSTICS = ")
        json.dump(payload, handle, separators=(",", ":"))
        handle.write(";\n")
    print(f"[Saved encoding diagnostics to {js_path}]")


def run_all_register_sizes(
    output_json_path: Optional[str] = "benchmarks/encoding_diagnostics.json",
    js_path: Optional[str] = "assets/encoding_diagnostics.js",
) -> dict:
